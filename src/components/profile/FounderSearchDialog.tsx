...
interface FounderSearchDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (userId: string) => Promise<void>;
  existingFounderIds: string[];
}

export function FounderSearchDialog({ isOpen, onClose, onSelect, existingFounderIds }: FounderSearchDialogProps) {
  const { user } = useAuth();
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const followers = (followersRes.data || []).map((f: any) => f.profiles);
      const following = (followingRes.data || []).map((f: any) => f.profiles);

      // Unique profiles
      const combined = [...followers, ...following].reduce((acc: ProfileSummary[], curr: any) => {
        if (!curr) return acc;
  ...
  const handleAdd = async (userId: string) => {
    if (existingFounderIds.includes(userId)) {
      toast.info('Este usuario ya es co-founder');
      return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden sm:rounded-2xl border-none shadow-2xl flex flex-col max-h-[85vh]">
        <DialogHeader className="px-6 pt-6 pb-4 bg-muted/30">
          ...
          <DialogDescription>
            Busca a un usuario por nombre o username para invitarlo como co-founder de esta app.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="p-4 space-y-1">
            ...
            {isSearching || (isLoading && searchTerm.trim().length < 2) ? (
              ...
              displayList.map((profile) => {
                const isAlreadyFounder = existingFounderIds.includes(profile.id);
                ...
                return (
                  ...
                  >
                    ...
                    <Button
                      size="sm"
                      variant={isAlreadyFounder ? "secondary" : "ghost"}
                      className={`h-8 px-3 gap-1.5 text-xs transition-all ${!isAlreadyFounder && 'hover:bg-primary hover:text-primary-foreground group-hover:opacity-100'}`}
                      onClick={() => !isAlreadyFounder && handleAdd(profile.id)}
                      disabled={addingId === profile.id || isAlreadyFounder}
                    >
                      {addingId === profile.id ? (
                      ...
                      {isAlreadyFounder ? 'Ya es founder' : 'Añadir'}
                    </Button>
                  </div>
                );
              })
              ...
              <div className="text-center py-12 px-6">
                ...
                <p className="text-sm text-muted-foreground">No se encontraron usuarios para "{searchTerm}"</p>
              </div>
            ) : (
              <div className="text-center py-12 px-6">
                <p className="text-sm text-muted-foreground">Empieza a escribir para buscar usuarios</p>
              </div>
            )}
          </div>
        </ScrollArea>
