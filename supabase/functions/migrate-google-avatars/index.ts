...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`≡ƒÜÇ Starting migration for admin: ${user.email}`);
    ...
    for (const profile of profiles) {
      try {
        ...
        console.log(`≡ƒöä Migrating ${profile.id}...`);
        ...
        successCount++;
        console.log(`Γ£à Success for ${profile.id}`);
      } catch (err) {
        console.error(`Γ¥î Failed for ${profile.id}:`, err);
        failCount++;
      }
    }
