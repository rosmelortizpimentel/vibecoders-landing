...
-- 1. Add founder_welcome_seen column
ALTER TABLE public.user_subscriptions 
ADD COLUMN IF NOT EXISTS founder_welcome_seen boolean NOT NULL DEFAULT false;

-- 2. Add RLS policy so users can update their own founder_welcome_seen
CREATE POLICY "Users can update own welcome seen" 
ON public.user_subscriptions 
FOR UPDATE 
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

-- 3. Update founder_numbers +20
UPDATE public.user_subscriptions 
SET founder_number = founder_number + 20 
WHERE founder_number IS NOT NULL;

-- 4. Update assign_founder_tier to use MAX instead of COUNT
